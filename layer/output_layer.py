...
class MLP(tf.keras.Model):
    def __init__(self, hidden_size):
        ...
        self.hidden_size = hidden_size
        self.dense1 = tf.keras.layers.Dense(units=hidden_size, activation=tf.nn.relu, dropout = 0.1)

        self.dense2 = tf.keras.layers.Dense(units=hidden_size, activation=tf.nn.relu,dropout = 0.1)
        self.dense3 = tf.keras.layers.Dense(units=hidden_size, activation=tf.nn.relu,dropout = 0.1)
...
class MLP2(tf.keras.Model):
    def __init__(self, hidden_size):
        ...
        self.hidden_size = hidden_size
        self.dense1 = tf.keras.layers.Dense(units=5*hidden_size, activation=tf.nn.relu)
        self.dense2 = tf.keras.layers.Dense(units=3*hidden_size, activation=tf.nn.relu)
        self.dense3 = tf.keras.layers.Dense(units=2*hidden_size, activation=tf.nn.relu)
        self.dense4 = tf.keras.layers.Dense(units=hidden_size, activation=tf.nn.relu)
        self.dense5 = tf.keras.layers.Dense(units=hidden_size, activation=tf.nn.softmax)

# with relu (all units) c_index is higher 0.4 buy accuracy is lower 0.5
    def call(self, input_x_train, training=None, mask=None):
        x_1 = tf.keras.layers.BatchNormalization()(self.dense1(input_x_train))
        x_2 = tf.keras.layers.BatchNormalization()(self.dense2(x_1))
        x_3 = tf.keras.layers.BatchNormalization()(self.dense3(x_2))
        x_4 = tf.keras.layers.BatchNormalization()(self.dense4(x_3))
        x_5 = tf.keras.layers.BatchNormalization()(self.dense5(x_4))
